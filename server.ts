...
// Parse JSON bodies (with reasonable limit for base64 images)
...
// Multi-model fallback helper for 503 / high demand resiliency
...
function parseJSONResponse(rawText: string) {
  ...
  try {
    ...
    // Attempt repairs for trailing commas before } or ]
...
// Fallback high-quality meal analysis data
function getSampleAnalysis(mealNameHint: string, ageTier: string = "classes-4-7") {
  const cleanHint = mealNameHint && mealNameHint !== "School Canteen Lunch Meal Tray" && mealNameHint !== "School Meal"
    ? mealNameHint.charAt(0).toUpperCase() + mealNameHint.slice(1)
    : "Chef's Special School Canteen Tray";
  ...
  let title = cleanHint.includes("Tray") || cleanHint.includes("Lunch") || cleanHint.includes("Meal") || cleanHint.includes("Captured") 
    ? cleanHint 
    : `${cleanHint} Balanced Lunch Tray`;
  let items = [
    {
      id: "item-1",
    ...
    {
      id: "item-2",
      name: "Sautéed Spinach, Carrots & Peas Curry",
      category: "Vegetables",
  ...
  return {
    ...
    ageTierExplanations: {
      "classes-1-3": {
        headline: "🌟 Super Fuel for Playground Champions & Fun Explorers!",
        simpleDescription:
          "Today's yummy tray gives your body rocket fuel to run super fast during recess, strong bones for climbing monkey bars, and sharp eyes like a hawk!",
        portionTip:
          "Take 3 brave bites of your colorful veggies and drink all your warm soup/dal! Finish your apple slice for a sparkling smile.",
        powerBadges: [
        ...
        smartFact: "💡 Did you know? Eating colorful food gives you different superhero powers every day!",
      ...
      "classes-4-7": {
        ...
        simpleDescription:
          "This meal is engineered with complex slow-release carbohydrates for non-stop classroom concentration and rich plant protein to repair active muscles after sports practice.",
        portionTip:
          "Classes 4–7: Eat your full serving of lentils/protein and at least 1 cup of colorful veggies. Avoid skipping the fiber to prevent afternoon energy crashes.",
        powerBadges: [
        ...
        smartFact: "💡 Brain Fact: Your brain consumes 20% of your daily energy. Complex grains give it steady fuel without sugar spikes!",
      ...
      "classes-8-10": {
        ...
        simpleDescription:
          "A scientifically balanced macronutrient profile (55% complex carbs, 20% protein, 15% healthy fats, high dietary fiber) designed for peak cognitive endurance, metabolic balance, and sports recovery.",
        portionTip:
          "Teenagers need higher caloric density. Ensure you consume the full portion of legumes/protein for vital amino acids and fiber to maintain steady glycemic balance.",
        powerBadges: [
        ...
        smartFact: "💡 Nutrition Science: Combining Vitamin C with plant-based iron boosts iron bioavailability by up to 300%!",
...
Format your output strictly as a JSON object matching this schema:
{
  ...
  "canteenStaffNotes": "Practical culinary/nutritionist notes for canteen preparation"
    ...
    // Graceful fallback to rich sample
...
// 2. AI Menu Optimizer & Assistant Endpoint
...
Return strict JSON with this schema:
{
  ...
  "sdg12Checklist": ["Practical tip 1", "Practical tip 2", "Practical tip 3"]
    ...
    // Graceful fallback on API error/503
    const fallback = {
      overallGrade: "A-",
      summaryReview: "Good baseline balance, but mid-week menus show a slight dip in dark green vegetables and whole grain variety.",
      nutritionalScore: 88,
...
// 3. AI Food Waste Insights Endpoint
    ...
    if (!ai) {
      return res.json({
        ...
        data: {
          ...
          keyInsights: [
            {
              ...
              actionableFix: "Roast with light garlic-olive oil and a dusting of grated cheese, or purée into pasta sauce as 'Hulk Green Sauce'.",
          ...
          smartBoardStudentChallenge: "🏆 Clean Plate Club: Can our school achieve under 8% cafeteria waste this Friday? The winning class gets the Golden Apple Trophy!"
        },
        isSimulated: true,
      });
    }
...
app.post("/api/staff-chat", async (req, res) => {
  try {
    ...
    const chatContext = chatHistory.map((h: any) => `${h.role === 'user' ? 'Staff' : 'NutriBoard AI'}: ${h.text}`).join("\n");
